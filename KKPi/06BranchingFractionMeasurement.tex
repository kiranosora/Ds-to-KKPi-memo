...
\par{
    ...
    \begin{itemize}
        ...
        \item Background shape and fit range. For background shape and the fit range in the fit, the first-order Chebychev polynomial is used to replace the nominal ones and the fit  range of $[1.90, 2.03]$ GeV/$c^{2}$ for Cat. A and $[-0.14, 0.13]$ GeV/$c^{2}$ are changed to $[1.90, 2.02]$ GeV/$c^{2}$ and $[-0.13, 0.13]$ GeV/$c^{2}$ respectively. 
        ...
        \item Fit bias. The possible bias is estimated by the input/output check using the round 30-40 of generic MC, which is shown in Table~\ref{BR-IO}. 
