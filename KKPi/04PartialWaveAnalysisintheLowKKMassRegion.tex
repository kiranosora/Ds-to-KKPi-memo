...
\begin{figure*}[htbp]
 ...
 \caption{The distributions (Cat. \#1) of these three observables ((a) and (d)) $M_{rec}$, ((b) and (e)) $P_{rest}$ and  ((c) and (f)) $E_{\gamma}$ for( (a), (b) and (c)) signal and ( (d), (e) and (f)) sideband regions from data.}
\label{cat1_data}
\end{figure*}
...
\end{figure*}
    ...
    The fit to the signal $D_{s}$ invariant mass gives the background yield in Signal region is $735.7\pm30.0$, shown as in Fig.~\ref{MIPWA-ST}.
...
\begin{figure*}[htbp]
    ...
    \caption{ The projections of $m(K^{+}K^{-})$, $m(K^{-}\pi^{+})$, $m(K^{+}\pi^{+})$ from ``Sideband" for data (dots with error bars) and generic MC with signal events removed (red histogram) after BDTG requirement.}
...
\par{Assuming N is the number of events for a given mass interval $I=[m_{K^{+}K^{-}}; m_{K^{+}K^{-}} + dm_{K^{+}K^{-}}]$, we write the corresponding angular distributions in terms of the appropriate spherical harmonic functions is written
    ...
    \end{equation}
    where $L = 2 \ell_{max}$, and $\ell_{max}$ is the maximum orbital angular momentum quantum number required to describe the $K^{+}K^{-}$ system at $m_{K^{+}K^{-}}$ (e.g. $\ell_{max}$ =1 for S-, P-wave description); $\theta$ is the angle between the direction of $K^{+}$ and that of the $K^{+}K^{-}$ system in the $D_{s}^{+}$ rest frame. 
...
\par{
    ...
    \begin{itemize}
        ...
        \item \uppercase\expandafter{\romannumeral5} Fit range. We vary the fit range from [0.988, 1.15] GeV/$c^{2}$ to [0.988, 1.145] GeV/$c^{2}$ and the shift of $m_{0}$ and $\Gamma_{0}$ are 0.002 GeV/$c^{2}$ and 0.003 GeV/$c^{2}$.  
    ...
    \begin{equation}
        \begin{array}{lr}
            ...
            \Gamma_{\alpha} = (0.272 \pm 0.040_{stat} \pm 0.024_{sys})\ {\rm GeV}/c^{2}, &
